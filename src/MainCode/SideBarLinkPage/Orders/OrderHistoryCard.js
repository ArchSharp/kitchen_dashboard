...
function OrderHistoryCard({ order }) {
  ...
  const { orders } = useAppSelector(selectKitchen);
  const cardStyle = {
    margin: "1rem",
    width: "45rem",
    height: "650px",
  ...
  const totalSum =
    order && Array.isArray(order.dishes)
      ? order.dishes.reduce((total, dishItem) => total + dishItem.price, 0)
      : 0;

  return (
    <div
      style={{ marginLeft: "7rem", display: "flex", justifyContent: "center" }}
    >
          ...
          title={
            ...
            >
              ...
              <span style={{ marginLeft: "auto", fontSize: "14px" }}>
                {moment(historyOrder.date).format(
